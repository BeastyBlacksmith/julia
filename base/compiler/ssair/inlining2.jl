...
function cfg_inline_item!(item::InliningTodo, state::CFGInliningState)
    ...
    if need_split
        push!(state.new_cfg_blocks, BasicBlock(state.cfg.blocks[block].stmts,
            Int[], orig_succs))
        push!(state.split_targets, length(state.new_cfg_blocks))
    end
...
function assemble_inline_todo!(ir::IRCode, linetable::Vector{LineInfoNode}, sv::OptimizationState)
    ...
    for idx in 1:length(ir.stmts)
        ...
        if meth === false || length(meth) == 0
            case = maybe_make_invoke!(ir, idx, stmt.typ, atypes, sv, atype_unlimited, false, isapply, nothing)
            @assert case === nothing
            continue
        end
        ...
        for match in meth
            ...
            if isa(metharg, UnionAll)
                fully_covered = false
                continue
            end
            if !isdispatchtuple(metharg)
                matypes = Any[metharg.parameters...]
                nu = countunionsplit(matypes)
                if nu == 1 || nu > sv.params.MAX_UNION_SPLITTING
                    fully_covered = false
                    continue
                end
                for sig in UnionSplitSignature(matypes)
                    metharg′ = Tuple{sig...}
                    if !isdispatchtuple(metharg′)
                        fully_covered = false
                        continue
